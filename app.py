...
Готовый app.py:
- Квиз: WebApp (в ЛС) + fallback/браузер + чат-квиз (ForceReply в группах)
- Валидация: отправляем заявку ТОЛЬКО если все поля заполнены и контакт валиден
- Контакты: «✍️ Написать админу» (антиспам — 1 сообщение в N сек)
- Админка (только ADMIN_CHAT_ID): счётчики и уникальные пользователи
- Доставка лидов: хедер + карточка в LEADS_CHAT_ID (группа/канал), дублирование админу
- Вебхук + /healthz + статика /webapp/quiz (если есть папка webapp/quiz)
"""
...
async def cmd_set_leads(m: Message):
    ...
    parts = (m.text or "").split(maxsplit=1)
    if len(parts) < 2:
        return await m.answer("Использование: /set_leads -4908337162 ИЛИ /set_leads @channel")
    global LEADS_RAW
...
# HTTP-fallback для браузера (строгая валидация)
...
# fallback HTML /webapp/quiz с клиентской валидацией
...
<div class="card">
  ...
  <input id="contact" placeholder="@username или телефон/email">
...
# --- Error handler (aiogram 3.x signature: event, exception) ---
@dp.error()
